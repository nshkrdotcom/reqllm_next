defmodule ReqLlmNext.Wire.OpenAIChatTest do
  ...
  describe "encode_body/3" do
    ...
    test "encodes inline file content" do
      model = TestModels.openai()
      ...
      context =
        Context.new([
          Context.user([
            ...
            ContentPart.file("pdf-bytes", "manual.pdf", "application/pdf")
          ])
        ])
      ...
      context =
        Context.new([
          Context.user([
            ...
            ContentPart.document_file_id("file-123")
          ])
        ])
    ...
    test "encodes file url references from canonical document parts" do
      ...
      context =
        Context.new([
          Context.user([
            ContentPart.text("Summarize this remote PDF"),
            ContentPart.document_url("https://example.com/manual.pdf")
          ])
        ])
      ...
      assert Enum.at(content, 1) == %{
               type: "file",
               file: %{file_url: "https://example.com/manual.pdf"}
             }
    end
