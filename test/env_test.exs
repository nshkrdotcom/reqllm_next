defmodule ReqLlmNext.EnvTest do
  ...
  test "loads values from a local env file" do
    ...
    on_exit(fn ->
      File.rm_rf(path)
      System.delete_env("REQ_LLM_NEXT_ENV_TEST_A")
      System.delete_env("REQ_LLM_NEXT_ENV_TEST_B")
    end)

    Env.load(path)

    assert System.get_env("REQ_LLM_NEXT_ENV_TEST_A") == "alpha"
    assert System.get_env("REQ_LLM_NEXT_ENV_TEST_B") == "beta value"
  end
  ...
  test "does not override values already present in the shell environment" do
    ...
    System.put_env("REQ_LLM_NEXT_ENV_TEST_C", "from_shell")

    on_exit(fn ->
      File.rm_rf(path)
      System.delete_env("REQ_LLM_NEXT_ENV_TEST_C")
    end)

    Env.load(path)

    assert System.get_env("REQ_LLM_NEXT_ENV_TEST_C") == "from_shell"
  end
