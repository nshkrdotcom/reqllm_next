defmodule ReqLlmNext.Anthropic.MessageBatches do
  ...
  def results(%{"results_url" => url}, opts) when is_binary(url) do
    Client.jsonl_request(url, opts)
  end
  ...
  def results(batch_id, opts) when is_binary(batch_id) do
    Client.jsonl_request("/v1/messages/batches/#{batch_id}/results", opts)
  end
  ...
  def delete(batch_id, opts \\ []) when is_binary(batch_id) do
    Client.json_request(:delete, "/v1/messages/batches/#{batch_id}", nil, opts)
  end
end
